...
Since `CLINICAL-STATES-SUB-SHEETS-2026-08-14`, all four Clinical States sub-sheets shared the same `"SID-"` prefix on their ID column (a holdover from when they were one sheet) — so an ID like `SID-004` didn't say on its face whether it was a Phase, a Rhythm, a Sedation level, or a Medication state. Murph asked for a distinct prefix per sub-sheet: Phase → `PID-`, Rhythm → `RID-`, Sedation → `SID-` (unchanged), Medication → `MEDID-`.

In `app/admin/model.ts`, each sub-sheet's ID column (still keyed `stateId` internally — unchanged, since `knowledge/validation.ts`'s `idColumn` map and `app/clinical/requiredStates.ts`'s `row.stateId` read both key off that name, not the prefix) got its own `idPrefix` and a more specific label: "Phase ID"/`PID-`, "Rhythm ID"/`RID-`, "Sedation ID"/`SID-`, "Medication ID"/`MEDID-`. Nothing about how these IDs are used elsewhere changed — `knowledge/validation.ts`'s per-sheet duplicate-ID check and Abbreviated-Name-based reference matching (Required States/Required Clinical State) don't care what the prefix text is, only that it's present and consistent within its own sheet.

Medication's prefix was initially set to `MID-`, which collides with Maneuver Definitions' existing Maneuver ID prefix (`app/admin/model.ts`, `maneuverId` column — predates this change). Flagged to Murph, who asked for `MEDID-` instead — no ambiguity with Maneuver ID, still reads clearly as "Medication." Never caused a functional collision either way (ID uniqueness and reference lookups are always scoped to one sheet at a time in this app, never compared across sheets) — this was purely a readability fix.
