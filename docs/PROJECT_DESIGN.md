...
## Fix: Stale Multi-Select Values (e.g. Required States) Had No Way to Be Removed (fixed 2026-08-14)

Murph reported some Maneuver Definitions rows' Required States column held old free-text artifacts from before the column was tethered to a lookup list — left over from before `CLINICAL-STATES-SUB-SHEETS-2026-08-14` (or even earlier), and now failing the `"Unknown clinical state"` validation check described in `CLINICAL-STATES-VALIDATION-ALERT-2026-08-14` above. He couldn't clear them and didn't want to recreate each row.

Root cause was in `MultiSelectCell` (`app/admin/components/SpreadsheetTable.tsx`): the checkbox panel only ever rendered one `<label>` per entry in the column's *current, valid* `options` list — so a selected value no longer present in that list (renamed/deleted source row, or pre-lookup free text that was never a valid option to begin with) had no checkbox anywhere in the panel to uncheck. It was visible in the summary line (the raw stored value, comma-split, regardless of validity) but had no control that could remove it — the only way out was deleting the whole row, which is what Murph didn't want to do. New rows couldn't gain a fresh stale value going in — the checkbox list itself was already select-from-list-only — this was purely an existing-data cleanup gap.

Fix: `MultiSelectCell` now also computes `staleSelected` — the selected values that aren't in `options` — and renders them as their own checked, toggleable checkboxes below a divider ("No longer a valid option — uncheck to remove"), styled in amber (`.adminMultiSelectStaleHeading`/`.adminMultiSelectOptionStale` in `app/globals.css`) so they read as flagged rather than blending in as ordinary choices. Unchecking one removes it from the stored comma-separated value exactly like unchecking a valid option does — no new mechanism, just extending the same toggle to values `options.includes()` doesn't cover. This is a general fix, not specific to Required States — any multi-select lookup column with the same kind of drift gets the same recovery path.

Verification: `npx tsc --noEmit` and `npx eslint app/` both clean. Not yet manually confirmed in the browser against Murph's actual stale rows — he'll see the flagged checkboxes next time he opens the affected Maneuver Definitions rows' Required States cell.

<!-- CLINICAL-STATES-DISTINCT-ID-PREFIXES-2026-08-14 -->
## Clinical States Sub-Sheets: Distinct ID Prefix Per Category (implemented 2026-08-14)

Since `CLINICAL-STATES-SUB-SHEETS-2026-08-14`, all four Clinical States sub-sheets shared the same `"SID-"` prefix on their ID column (a holdover from when they were one sheet) — so an ID like `SID-004` didn't say on its face whether it was a Phase, a Rhythm, a Sedation level, or a Medication state. Murph asked for a distinct prefix per sub-sheet: Phase → `PID-`, Rhythm → `RID-`, Sedation → `SID-` (unchanged), Medication → `MID-`.

In `app/admin/model.ts`, each sub-sheet's ID column (still keyed `stateId` internally — unchanged, since `knowledge/validation.ts`'s `idColumn` map and `app/clinical/requiredStates.ts`'s `row.stateId` read both key off that name, not the prefix) got its own `idPrefix` and a more specific label: "Phase ID"/`PID-`, "Rhythm ID"/`RID-`, "Sedation ID"/`SID-`, "Medication ID"/`MID-`. Nothing about how these IDs are used elsewhere changed — `knowledge/validation.ts`'s per-sheet duplicate-ID check and Abbreviated-Name-based reference matching (Required States/Required Clinical State) don't care what the prefix text is, only that it's present and consistent within its own sheet.

**Flagging for Murph, not fixed:** Medication's new `MID-` prefix is identical to Maneuver Definitions' existing Maneuver ID prefix (`app/admin/model.ts`, `maneuverId` column — predates this change). This doesn't cause a functional collision — ID uniqueness and reference lookups are always scoped to one sheet at a time in this app, never compared across sheets — but it does mean `MID-001` alone, out of context, could refer to either a maneuver or a Medication clinical state. If that ambiguity matters in practice (e.g. in conversation, in exported reports, or when searching), let me know and I can pick a different Medication prefix.

**Migration note for Murph:** this only relabels new entries going forward — it doesn't touch existing data. Any rows already entered on the Phase/Rhythm/Medication sub-sheets under the old `SID-` prefix will now fail save/lock validation with "`<Sheet>`: `<ID column>` must begin with `\"<new prefix>\"`." (the same alert that now lists every issue at once, per `CLINICAL-STATES-VALIDATION-ALERT-2026-08-14`). The fix is a one-line text edit per row — renaming e.g. `SID-004` to `PID-004` in that row's ID cell — not a recreation of the row; nothing else about the row needs to change, and nothing that already references it by Abbreviated Name breaks, since those references were never ID-based to begin with (see `CLINICAL-STATES-SUB-SHEETS-2026-08-14`). Sedation entries are unaffected (`SID-` didn't change).
